...
const emptyForm = { name: "", description: "", mentor: "", startDate: "", endDate: "", status: "PLANNED", outcome: "", notes: "" };
...
export function ProjectsContent() {
  ...
  async function load() { const res = await fetch("/api/projects"); setItems(await res.json()); }
  useEffect(() => { load(); }, []);

  function openAdd() { setForm({ ...emptyForm }); setEditItem(null); setError(""); setOpenForm(true); }
  function openEdit(item: any) {
    setForm({ name: item.name, description: item.description || "", mentor: item.mentor || "", startDate: item.startDate ? item.startDate.slice(0, 10) : "", endDate: item.endDate ? item.endDate.slice(0, 10) : "", status: item.status, outcome: item.outcome || "", notes: item.notes || "" });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><Button onClick={openAdd}><Plus className="h-4 w-4" /> Add Project</Button></div>
      {items.length === 0 ? (
        <EmptyState title="No projects yet" description="Add your personal or team projects." action={<Button onClick={openAdd}><Plus className="h-4 w-4" /> Add Project</Button>} />
      ) : (
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["Project Name", "Mentor", "Start Date", "End Date", "Status", "Tasks", ""].map((h) => (
      ...
      <Dialog open={openForm} onClose={() => setOpenForm(false)} title={editItem ? "Edit Project" : "Add Project"} className="max-w-2xl">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2"><FormField label="Project Name" name="name" value={form.name} onChange={setField("name")} required /></div>
          ...
          <div className="col-span-2"><FormField label="Outcome" name="outcome" type="textarea" value={form.outcome} onChange={setField("outcome")} /></div>
      ...
      <ConfirmDialog open={deleteId !== null} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Project" description="Are you sure you want to delete this project?" />
    </div>
  );
}
